class App {
  ...
  addTodo = () => {
    ...
    // 1.여기서 title === "" 또는 content === "" 이면 alert 으로 경고를 띄우고 삽입되지 않게
    if (title.trim() === "" || content.trim() === "") {
      alert('필수값이 비어있습니다.');
      return;
    }
    // DOM 을 만들어서 listDom 의 자식으로 넣어준다.
    const idx = ++this.idx;
    ...
    this.listDom.appendChild(dom);
    // 2. 여기서 삽입이 된 후에는 titleInput 과 contentInput 에 입력한 on 내용이 지워지도록
